...
class NewsDatasetCategory(data.Dataset):
    def __init__(self, hyperParams, file_path):
        super(NewsDatasetCategory, self).__init__()
        self.hyperParams = hyperParams
        self.file_path = file_path
        self.behavior, self.clickHis = self.init_user_behavior()
        self.title_token, self.abstract_token, self.category_map, self.category_label, self.global_frequency = self.get_news()
    ...
    def __getitem__(self, item):
        ...
        """
        click_titles, click_abstracts, clicks_category_feature = self.get_click_news(item)
        candidate_titles, candidate_abstracts = self.get_candidates(item)
        candidate_labels = self.get_candidates_label()
        # shuffle the samples
        tmp = list(zip(candidate_titles, candidate_abstracts, candidate_labels))
        random.shuffle(tmp)
        candidate_titles, candidate_abstracts, candidate_labels = zip(*tmp)
        return torch.tensor(click_titles), torch.tensor(click_abstracts), \
               torch.tensor(candidate_titles), torch.tensor(candidate_abstracts),\
               torch.tensor(clicks_category_feature), torch.tensor(candidate_labels)
    ...
    def get_click_news(self, idx):
        ...
        try:
            ...
            clicks_category_feature = [self.category_label[self.category_map[id]] for id in
                                    self.behavior[idx][1][:news_len]]
            clicks_category_freq = [0] * len(self.global_frequency)
            for ca in clicks_category_feature:
                clicks_category_freq[ca] += 1
            total_num = sum(clicks_category_freq)
            for i in range(len(self.global_frequency)):
                clicks_category_freq[i] /= total_num * self.global_frequency[i]
        ...
        if len(click_news_title) < pos_num:
            ...
            padding_category_feature = [min(self.global_frequency) for _ in range(pos_num - len(click_news_title))]
            click_news_title = click_news_title + padding_news_title
            click_news_abstract = click_news_abstract + padding_news_abstract
            clicks_category_feature = clicks_category_feature + padding_category_feature
        return click_news_title, click_news_abstract, clicks_category_feature
    ...
    def get_candidates(self, idx):
        ...
        """
        title_len = self.hyperParams["data"]["titleLen"]
        abstract_len = self.hyperParams["data"]["wordLen"]
        ...
        zero_padding_title, zero_padding_abstract = [0] * title_len, [0] * abstract_len
        if len(candidate_titles) <= neg_num:
            padding_news_title = [zero_padding_title for _ in range(neg_num + 1 - len(candidate_titles))]
            padding_news_abstract = [zero_padding_abstract for _ in range(neg_num + 1 - len(candidate_abstracts))]
            candidate_titles = candidate_titles + padding_news_title
            candidate_abstracts = candidate_abstracts + padding_news_abstract
...
class NewsDatasetCategoryVal(data.Dataset):
    def __init__(self, hyperParams, file_path):
        super(NewsDatasetCategoryVal, self).__init__()
        self.hyperParams = hyperParams
        self.file_path = file_path
        self.behavior, self.clickHis = self.init_user_behavior()
        self.title_token, self.abstract_token, self.category_map, self.category_label, self.global_frequency = self.get_news()
...
if __name__ == "__main__":
    ...
    train_set = NewsDatasetCategory(hyperParams, "../data/val")
    count = 10
